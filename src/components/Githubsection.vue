<template>
  <div class="settings-section settings-card bg-card border border-[#1e2530] rounded-xl overflow-hidden">
    ...
    <div class="flex items-center gap-3 px-4 md:px-5 pt-4 md:pt-5 pb-3 border-b border-[#1e2530]">
      <div class="w-8 h-8 rounded-lg bg-indigo-900/40 border border-indigo-800/40 flex items-center justify-center shrink-0">
    ...
    <div class="px-4 md:px-5 py-4 space-y-4">
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
        ...
        <div class="space-y-1.5">
          ...
          <div class="relative">
            <input :value="patValue" @input="emit('update:patValue', $event.target.value)"
              ...
              placeholder="ghp_••••••••••••••••••••"
              class="settings-input pat-input w-full bg-[#0d1117] border border-[#30363d]
                     rounded-lg px-3 py-2.5 text-xs text-gray-300 outline-none transition-all pr-10
                     font-mono" />
            <button @click="showPat = !showPat"
              ...
              <img v-if="!showPat" :src="passShowIcon" width="14" height="14" alt="" class="opacity-60 invert brightness-0" />
        ...
        <div class="space-y-1.5">
          ...
          <div class="relative">
            <input :value="repoUrl" @input="emit('update:repoUrl', $event.target.value)"
              type="text" placeholder="https://github.com/username/repo"
              class="settings-input w-full bg-[#0d1117] border border-[#30363d]
                     rounded-lg px-3 py-2.5 text-xs text-gray-300 outline-none transition-all pr-8
                     font-mono" />
            <div class="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-emerald-500 flex items-center justify-center">
...
const emit = defineEmits(['update:patValue', 'update:repoUrl'])
